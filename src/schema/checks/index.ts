...
export const fromSchema =
    <B extends json.Object>(c: Context<B>) =>
    (s: Schema): Check<B> =>
        wrapOptional(
            s,
            addCustom(
                ...
                <Check<B>>match(s)
                    .caseOf(
                        schema.objectShapeWithAllProperties,
                        fromMapObject(c)
                    )
                    .caseOf(schema.objectShapeWithProperties, fromObject(c))
                    .caseOf(
                        schema.objectShapeWithAdditionalProperties,
                        fromMap(c)
                    )
...
const wrapOptional = <B extends json.Object>(
    s: Schema,
    ch: Check<B>
): Check<B> => (s.optional === true ? <Check<B>>optional(ch) : ch);

const addCustom = <B extends json.Object>(
    c: Context<B>,
    s: Schema,
    ch: Check<B>
): Check<B> =>
...
const fromMap =
    <B extends json.Object>(c: Context<B>) =>
    ({ additionalProperties, $checks }: ObjectType) =>
        every(
            records.isRecord,
...
const fromMapObject =
    ...
    ({ properties, additionalProperties, $checks }: ObjectType) =>
        and(
            <Precondition<json.Value, json.Object>>records.isRecord,
            and(
                records.union(map(<Type>properties, fromSchema(c))),
                and(
                    records.map(fromSchema(c)(<Type>additionalProperties)),
                    <Precondition<json.Object, B>>(
                        specs2Checks(c.providers)($checks || [])
                    )
                )
            )
