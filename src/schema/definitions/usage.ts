 ...
 */
export const isRef = (s: Schema) =>
    isRecord(s) && typeof s['type'] === 'string' && s['type'][0] === SYMBOL;
...
export const evalUsage =
    ...
    (def: Usage): Result<Usage, Definition> =>
...
export const evalUsages =
    (defs: Definitions) =>
    (work: Usages): Result<Usages, Definitions> =>
...
const pullObject = (work: Usages) => (path: string) => (schema: ObjectType) => {
...
const hasPullableAdditionalRefs = (schema: ObjectType) =>
    isRecord(schema.additionalProperties) && isRef(schema.additionalProperties);

const pullArray = (work: Usages) => (path: string) => (schema: ArrayType) =>
    isRef(schema.items)
        ? merge(work, { [join(path, 'items')]: snip(schema.items) })
        : work;

const pullSum = (work: Usages) => (path: string) => (schema: SumType) =>
    reduce(schema.variants, work, (p, c: Schema, k) =>
        pull(p)(join(path, 'variants', k))(c)
    );

const pullOther = (work: Usages) => (path: string) => (schema: Schema) =>
