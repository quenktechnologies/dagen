...
import {
    PrimFailure,
    DualFailure
} from '@quenk/preconditions/lib/result/failure';
...
describe('checks', () => {
    describe('Context', () => {
        describe('addChecks', () => {
            it('should source checks from a map', () => {
                let ctx = new Context();
                let r = map(
                    <any>ctx.addChecks(schema.all).checks,
                    (c: Check<Value>, k: string) =>
                        c(samples.all[k]).takeRight()
                );
            ...
            it('should allow recursion', () => {
                ...
                let r = map(
                    <any>ctx.addChecks(schema.rSchema).checks,
    ...
    describe('fromSchema', () => {
        it('should produce a check for string types', () => {
            must(
                fromSchema(ctx)(schema.string)(samples.string).takeRight()
            ).equal(samples.string);

            must(
                fromSchema(ctx)(schema.string)(samples.number).takeLeft()
            ).be.instance.of(PrimFailure);
        });

        it('should produce a precondition for number types', () => {
            must(
                fromSchema(ctx)(schema.number)(samples.number).takeRight()
            ).equal(samples.number);

            must(
                fromSchema(ctx)(schema.number)(samples.string).takeLeft()
            ).be.instance.of(PrimFailure);
        });

        it('should produce a check for boolean types', () => {
            must(
                fromSchema(ctx)(schema.boolean)(samples.boolean).takeRight()
            ).equal(samples.boolean);

            must(
                fromSchema(ctx)(schema.boolean)(samples.string).takeLeft()
            ).be.instance.of(PrimFailure);
        });
        ...
        it('should produce a precondition for object types', () => {
            ...
            must(
                fromSchema(ctx)(schema.object)(samples.object).takeRight()
            ).equate(samples.object);

            must(fromSchema(ctx)(schema.object)(optionaled).takeRight()).equate(
                optionaled
            );

            must(
                fromSchema(ctx)(schema.object)(wrong).takeLeft().explain()
            ).equate({
                profile: {
                    left: {
                        left: '',
        ...
        it('should produce a precondition for array types', () => {
            must(
                fromSchema(ctx)(schema.array)(samples.array).takeRight()
            ).equate(samples.array);

            must(
                fromSchema(ctx)(schema.array)(samples.array[0]).takeLeft()
            ).be.instance.of(PrimFailure);
        });

        it('should produce a precondition for an array of numbers', () => {
            must(
                fromSchema(ctx)(schema.arrayOfNumbers)([0, 1, 2]).takeRight()
            ).equate([0, 1, 2]);

            must(
                fromSchema(ctx)(schema.arrayOfNumbers)(['0', '1', '2'])
                    .takeLeft()
                    .explain()
            ).equate({
                '0': 'isNumber',
        ...
        it('should produce a precondition for sum types', () => {
            must(fromSchema(ctx)(schema.sum)(samples.string).takeRight()).equal(
                samples.string
            );

            must(fromSchema(ctx)(schema.sum)(samples.number).takeRight()).equal(
                samples.number
            );

            must(
                fromSchema(ctx)(schema.sum)(samples.boolean).takeRight()
            ).equal(samples.boolean);

            must(
                fromSchema(ctx)(schema.sum)(samples.object).takeLeft()
        ...
        it('should produce a precondition for ref types ', () => {
            ...
            must(
                fromSchema(ps)(schema.object)(samples.object).takeRight()
            ).equate(samples.object);

            must(
                fromSchema(ps)(schema.object)(samples.string).takeLeft()
            ).be.instance.of(PrimFailure);
        });

        it('should produce a precondition for ref types ', () => {
            must(
                fromSchema(ctx)(schema.withChecks)(
                    samples.withChecks
                ).takeRight()
            ).equate(set('profile', 'uname', samples.withChecks));

            must(
                fromSchema(ctx)(schema.withChecks)(samples.withChecksWrong)
                    .takeLeft()
                    .explain()
            ).equate({
                name: 'eq',
        ...
        it('should not mangle schema has nested arrays', () => {
            must(
                fromSchema(ctx)(schema.mangledNestedArraySchema)(
                    samples.mangledNestedArraySample
                ).takeRight()
