 ...
 */
exports.loadSchema = (path) => path ?
    exports.load(path)
        .catch(e => future_1.raise(new Error(`Error loading schema "${path}": "${e.message}"`))) :
    future_1.pure({ type: 'object', title: 'Object', additionalProperties: {} });
/**
...
exports.loadPlugins = (ctx, paths) => monad_1.doN(function* () {
    ...
    let futs = mods.map((m) => future_1.attempt(() => {
 ...
 */
exports.loadChecks = (paths, list = []) => exports.loadN(paths)
    .chain((s) => future_1.pure(s.concat(list).map(checks_1.fromSchema(new checks_1.Context()))))
    .catch(checkErr);
const checkErr = (e) => future_1.raise(new Error(`Failed loading one or more checks: "${e.message}"`));
/**
 * loadChecks from an array of paths.
 */
exports.convertCheckSchema = (s = []) => future_1.pure(s.map(checks_1.fromSchema(new checks_1.Context())));
/**
