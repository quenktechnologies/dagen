...
const fromObject = (c) => ({ properties, $checks }) => preconditions_1.every(records.isObject, records.union(record_1.map(properties, exports.fromSchema(c))), spec_1.specs2Checks(c.providers)($checks || []));
const fromMap = (c) => ({ additionalProperties, $checks }) => preconditions_1.every(records.isObject, records.map(exports.fromSchema(c)(additionalProperties)), spec_1.specs2Checks(c.providers)($checks || []));
const fromMapObject = (c) => ({ properties, additionalProperties, $checks }) => preconditions_1.every(records.isObject, records.union(record_1.map(properties, exports.fromSchema(c))), records.map(exports.fromSchema(c)(additionalProperties)), spec_1.specs2Checks(c.providers)($checks || []));
const fromArray = (c) => ({ items }) => preconditions_1.every(arrays.isArray, console.error(items) ||
    arrays.map(exports.fromSchema(c)(items)), arrays.map(spec_1.specs2Checks(c.providers)(items.$checks || [])));
const fromSum = (c) => ({ variants }) => record_1.reduce(record_1.map(variants, exports.fromSchema(c)), preconditions_1.fail(''), preconditions_1.or);
const fromRef = (c) => ({ ref }) => refPrec(c)(ref);
const refPrec = (c) => (p) => (v) => c.checks.hasOwnProperty(p) ?
    c.checks[p](v) :
    result_1.failure(`unknown ref "${p}" known: ${record_1.keys(c.checks)}`, v);
