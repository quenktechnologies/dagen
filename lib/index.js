 ...
 */
exports.readModule = (context) => (path) => {
    let p = path_1.isAbsolute(path) ? path :
        startsWith('.', String(path)) ?
            require.resolve(path_1.join(process.cwd(), path)) :
            path;
    let m = require.main.require(polate_1.polate(p, context, exports.polateOptions));
 ...
 */
exports.readPlugin = (ctx) => (path) => {
    if ((path[0] === '[') && (path[path.length - 1] === ']')) {
        let parts = path.slice(1, path.length - 1).split(' ');
        let realPath = parts[0];
        let argv = parts.slice(1).join(' ').trim();
        return checks_1.pluginModuleCheck(exports.readModule(ctx)(realPath))
            .map(_resolvePlugin(argv))
            .orRight(_rejectPlugin(realPath))
            .takeRight();
    }
    else {
        return checks_1.pluginModuleCheck(exports.readModule(ctx)(path))
            .map(_resolvePlugin(''))
...
const _sets2Context = (value) => value.reduce((p, kvp) => {
    let [path, value] = kvp.split('=');
    return property_seek_1.set(path, startsWith('require://', value) ?
        exports.readModule(p)(value.split('require://')[1]) : value, p);
}, {});
/**
...
exports.options2Program = (options) => (document) => Promise
    ...
    context: util_1.fuse(_sets2Context(options.sets), options.contexts.reduce((p, c) => util_1.fuse(p, exports.readModule(p)(c)), {})),
    options,
    plugins,
    after: []
}));
const _refError = (path) => (e) => exports.reject(new Error(`Error '${path}': ${e.stack}`));
const _fuseRef = (path, ref) => (doc) => exports.readRef(path_1.resolve(path, ref))
    .then(ref => exports.resolve(util_1.fuse(doc, ref)));
const _fuseRefList = (path, list) => (doc) => exports.readRefs(pathsResolve(path)(list))
...
exports.resolveRef = (path) => (json) => util_1.reduce(json, (previous, current, key) => (key === exports.REF) ?
    (Array.isArray(current) ?
        previous
            .then(_fuseRefList(path_1.dirname(path), current)) :
        previous
            .then(_fuseRef(path_1.dirname(path), current))) :
    Array.isArray(current) ?
        (previous
 ...
 */
exports.readRefs = (paths) => Promise.all(paths.map(exports.readRef));
/**
