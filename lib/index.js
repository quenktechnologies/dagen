...
exports.readPlugin = (path) => {
    if ((path[0] === '[') && (path[path.length - 1] === ']')) {
        let point = path.indexOf(',');
        let realPath = path.slice(1, point);
        let args = `[${path.slice(point + 1, path.length - 1)}]`;
        try {
            let tmp = JSON.parse(args);
            if (Array.isArray(tmp))
                return Promise.resolve(exports.readModule(realPath)(tmp));
        }
        catch (e) {
        ...
        return exports.readModule(path)();
    }
};
/**
 * readFile wrapper.
...
const _sets2Context = (value) => value.reduce((p, kvp) => {
    ...
    return property_seek_1.set(path, startsWith('require://', value) ?
        exports.readModule(value.split('require://')[1]) : value, p);
}, {});
/**
...
exports.options2Program = (options) => (document) => Promise
    ...
    context: util_1.fuse(_sets2Context(options.sets), options.contexts.reduce((p, c) => util_1.fuse(p, exports.readModule(c)), {})),
    options,
    plugins,
    after: []
}));
